...
rm(list=ls())
setwd("/data/shengf2/simu_mtop/power_hert/result_four")
...
res3 = colMeans(p_ftop<=5E-08,na.rm = T)
res4 = colMeans(p_topo<=5E-08) 
nfiles

res = rbind(res1,res3,res4)
row.names(res) = c("MTOP","ftop","topo")
res
write.csv(res,paste0("../results_four_hert_",nfiles,".csv"), row.names =TRUE)
